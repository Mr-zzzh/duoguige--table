...
use app\mobile\controller\Paynotification;
use think\Db;
...
class Pay extends Common
{
    ...
    public function pay($orderinfo){
        ...
        }else if (!empty($orderinfo['notify']) && $orderinfo['paytype'] != 1){
    ...
    public function paynotify($orderdata = ''){
        ...
        if ($data['paystatus'] == 1){
            //支付成功
            $this->notify_success($orderdata);
            $message = '订单支付成功';
        }else if ($data['paystatus'] == -1){
            //支付失败
            $this->notify_fail($orderdata);
            $message = '订单支付失败';
        }else{
            //异常
            $message = '订单异常';
