...
if (!function_exists('login_comc')) {
    function login_comc() {
        $comc = array('admin/admin/login', 'admin/admin/register', 'mobile/index/login', 'mobile/index/login_code', 'mobile/index/insurance', 'mobile/index/search', 'mobile/index/city', 'mobile/user/code', 'mobile/user/register', 'mobile/goods/goodscate', 'mobile/goods/index', 'mobile/goods/read', 'mobile/brand/index', 'mobile/brand/branddatum', 'mobile/brand/read', 'mobile/invite/salary', 'mobile/invite/experience', 'mobile/fault/transition', 'mobile/question/index', 'mobile/question/read', 'mobile/question/answer', 'mobile/news/index', 'mobile/technician/index', 'mobile/technician/read', 'mobile/technician/question');
        return $comc;
    }
}
//公共方法(无需权限限制)
...
if (!function_exists('check_rule')) {
    function check_rule() {
        ...
        if ($member['type'] == 0) {
            ...
            $key  = 'position_rule_' . $member['pid'];
            $rule = \think\Cache::get($key) ?: [];
            if (empty($rule)) {
                $pModel = new \app\admin\model\Position();
                $rule   = $pModel->where(array('id' => $member['pid']))->value('rule');
                $rule   = explode(',', $rule);
                \think\Cache::set($key, $rule, 3600);
            }
            if (in_array($path, comc())) {
                return true;
            }
...
if (!function_exists('StEntime')) {

    function StEntime($time = 'd', $type = 's') {
        if ($time == 'd' && $type == 's') {
        ...
        if ($time == 'lld' && $type == 's') {
            return mktime(0, 0, 0, date('m'), date('d') - 2, date('Y'));
        }
        if ($time == 'lld' && $type == 'e') {
            return mktime(0, 0, 0, date('m'), date('d') - 1, date('Y')) - 1;
        }
        ...
        if ($time == 'llw' && $type == 's') {
            return mktime(0, 0, 0, date('m'), date('d') - date('w') + 1 - 14, date('Y'));
        }
        if ($time == 'llw' && $type == 'e') {
            return mktime(23, 59, 59, date('m'), date('d') - date('w') + 7 - 14, date('Y'));
        }
        if ($time == 'm' && $type == 's') {
        ...
        if ($time == 'y' && $type == 'e') {
            return strtotime(date("Y", time()) . "-12" . "-31");
        }
    }
...
//消息提醒阅读修改
if (!function_exists('inform_read')) {
    function inform_read($id) {
        global $member;
        if (empty($id)) {
            return false;
        }
        ...
        $informModel = new \app\admin\model\Inform();
        $res         = $informModel->where(array('id' => ['in', $ids], 'mid' => $member['id']))->update(array('status' => 1));
        return $res;
    }
}
...
//公司规模获取
if (!function_exists('clientele_scale')) {
    function clientele_scale($id = 0) {
        $res  = array();
        $data = array('1' => '0-10人', '2' => '11-20人', '3' => '21-50人', '4' => '51-100人', '5' => '101-200人', '6' => '201-500人', '7' => '500人以上');
        if (!empty($id)) {
            if (is_numeric($id)) {
                return $data[$id];
            } else {
                return array_search($id, $data) ?: 1;
            }
        } else {
            foreach ($data as $k => $v) {
